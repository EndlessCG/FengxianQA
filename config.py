...
faq_runner_config = dict(
    ...
    table_name="t_nlp_qa_faq_test", # mysql表名
...
ner_model_config=dict(
    ...
    train=dict(
        ...
        data_dir="input/data/ner", # 输入数据目录
        pre_train_model_file="input/pretrained_BERT/bert-base-chinese-model.bin", # 预训练模型路径
        vob_file="input/pretrained_BERT/bert-base-chinese-vocab.txt", # 预训练模型词汇表路径
        model_config_file="input/pretrained_BERT/bert-base-chinese-config.json", # 预训练模型配置文件路径
        output_dir="models/NER/ner_output", # 模型保存路径
    ...
    test=dict(
        model_path='models/NER/ner_output/best_ner.bin', # 待测试模型路径
        data_dir="input/data/ner/", # 训练过程中生成的cached测试数据所在文件夹
        raw_data_path="input/data/ner/test.txt", # 测试数据原文件路径
        vob_file="input/pretrained_BERT/bert-base-chinese-vocab.txt", # 待测试模型词汇表路径
        model_config_file="input/pretrained_BERT/bert-base-chinese-config.json", # 待测试模型配置文件路径
        max_seq_length=128, # 最大序列长度
    )
)
...
sim_model_config=dict(
    ...
    train=dict(
        ...
        vob_file="input/pretrained_BERT/bert-base-chinese-vocab.txt", # 预训练模型词汇表路径
        model_config_file="input/pretrained_BERT/bert-base-chinese-config.json", # 预训练模型配置文件路径
        output_dir="models/SIM/sim_output", # 模型保存路径
        output_model_file="best_sim.bin",
        max_seq_length=128, # 最大序列长度
        train_batch_size=32, # 训练batch size
        eval_batch_size=32, # 训练时测试batch size
        gradient_accumulation_steps=4, # 梯度累加步数
    ...
    test=dict(
        ...
        model_path='models/SIM/sim_output/best_sim.bin', # 待测试模型路径
...
faq_model_config=dict(
    ...
    finetune=dict(
        ...
        batch_size=32, # finetune batch size
        epoch=30, # finetune epoch数
